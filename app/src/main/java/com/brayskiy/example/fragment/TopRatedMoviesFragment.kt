...
import com.brayskiy.example.contract.TopRatedMoviesFragmentContract
...
class TopRatedMoviesFragment : BaseFragment<TopRatedMoviesFragmentContract.IFragmentAdapter,
        TopRatedMoviesFragmentContract.IFragmentHandler>(), TopRatedMoviesFragmentContract.IFragmentAdapter {

    protected lateinit var picasso: Picasso @Inject set

    protected lateinit var injector: ActivityComponent @Inject set

    override val layoutId: Int = R.layout.top_rated_movies_fragment

    private lateinit var topRatedMovies: RecyclerView

    private var topRatedMax = Int.MAX_VALUE
    private var loading = false
    private var currentPage = 1

    override fun setupFragmentView(view: View) {
        topRatedMovies = view.findViewById(R.id.top_rated_movies_recycler)

        topRatedMovies.layoutManager = GridLayoutManager(context, 2)
    ...
    override fun injectFragment(injector: ActivityComponent) {
        ...
        handler.bindData().observe(this, Observer { result ->
            // TODO
        })
    }

    override fun onTopRatedSuccess(moviesData: MoviesData, provider: CardHandlerProvider<MovieSummaryHandler>) {
        if (moviesData.results.isEmpty()) {
            ...
            if (topRatedMovies.adapter == null) {
                topRatedMovies.adapter = CardsViewAdapter(context!!, cardsList)
            } else {
                (topRatedMovies.adapter as CardsViewAdapter).addCardsList(cardsList)
            }
        }
    }

    override fun onTopRatedError(throwable: Throwable) {
